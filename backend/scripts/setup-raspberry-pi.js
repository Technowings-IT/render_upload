// scripts/setup-raspberry-pi.js - Setup Raspberry Pi for AGV Map Deployment
...
async function setupRaspberryPi() {
    console.log('🚀 Setting up Raspberry Pi for AGV Map Deployment...');
    ...
    return new Promise((resolve, reject) => {
        conn.on('ready', async () => {
            ...
            try {
                ...
                await executeCommand(conn, `mkdir -p ${homeDir}/.agv-fleet`);
                ...
                console.log('🔄 Attempting to start AGV map loader service...');
                try {
                    await executeCommandWithSudo(conn, 'systemctl start agv-map-loader.service', SSH_CONFIG.password);
                    await executeCommand(conn, 'systemctl status agv-map-loader.service --no-pager --user || true');
                } catch (serviceError) {
                ...
                console.log('   • Service: agv-map-loader.service (may require manual setup)');
                console.log(`   • Config: ${homeDir}/.agv-fleet/config.json`);
...
function createDefaultMapScript(homeDir) {
    return `#!/bin/bash
# AGV Fleet Map Loader Script
# Auto-generated by setup-raspberry-pi.js

MAP_DIR="${homeDir}/ros2_ws/src/nav2_bringup/maps"
ROS_WS="${homeDir}/ros2_ws"
CONFIG_FILE="${homeDir}/.agv-fleet/config.json"
LOG_FILE="${homeDir}/.agv-fleet/map-loader.log"
...
Description=AGV Fleet Map Loader
...
WantedBy=multi-user.target"
    ...
    echo "$service_content" | sudo tee /etc/systemd/system/agv-map-loader.service > /dev/null
    sudo systemctl daemon-reload
    sudo systemctl enable agv-map-loader.service
