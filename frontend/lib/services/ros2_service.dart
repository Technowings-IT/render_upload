...
class EnhancedROS2Service {
  ...
  void _completeCommand(String commandId, Map<String, dynamic> response) {
    if (_activeCommands.containsKey(commandId)) {
      final command = _activeCommands[commandId]!;
      command.status = response['success'] ? CommandState.completed : CommandState.failed;
  ...
  void _updateCommandStatus(Map<String, dynamic> data) {
    ...
    if (processUpdate != null) {
      _activeCommands.values.where((cmd) => 
  ...
  Future<CommandResult> stopProcess(
    ...
    try {
      ...
      if (!validProcesses.contains(processName)) {
        throw Exception('Invalid process name. Valid options: ${validProcesses.join(', ')}');
      }
  ...
  Future<CommandResult> uploadAndConvertMap({
    ...
    try {
      print('📂 Uploading and converting map: $mapName - Command ID: $commandId');
  ...
  Future<CommandResult> _waitForCommandCompletion(
    ...
    while (DateTime.now().difference(startTime) < timeout) {
      if (!_activeCommands.containsKey(commandId)) {
        ...
        final completedCommand = _commandHistory
            .where((cmd) => cmd.id == commandId)
            .firstOrNull;
    ...
    // Timeout reached
    _failCommand(commandId, 'Command timeout after ${timeout.inSeconds} seconds');
  ...
  String _generateRandomString(int length) {
    ...
    return List.generate(length, (index) => 
      chars[(DateTime.now().microsecond + index) % chars.length]
    ).join();
  }
  ...
  Map<String, dynamic> getStatistics() {
    ...
    final recentCommands = _commandHistory.where((cmd) => 
